...
Одним из возможных способов задать булеву функцию $ f $ от $ n $ переменных является задание ее таблицы истинности, количество строк которой равно $ 2^n $. Отсюда следует, что мощность множества всех булевых функций от $ n $ переменных $ \left| \mathbb{P}_2(n) \right| $ равна $ 2^{2^n} $, поскольку на каждую строку таблицы истинности булева функция может ответить нулем или единицей, то есть элементами из множества $ \{ 0,\ 1 \} $ мощности 2.
...
Интерес задачи минимизации обобщенных полиномов представляют преимущества, которыми они в сравнении с ДНФ. С практической точки зрения можно выделить экспериментально наблюдаемые меньшие размеры выражения \cite{exorcism4} (для метрики, отвечающей количеству конъюнктов в выражении).
...
В курсовой работе за третий курс был предложен метод дискретного аналога покоординатного спуска с прибавлением нулевых обобщенных полиномов.



Этот эвристический подход продолжает исследоваться в рамках данной работы.
